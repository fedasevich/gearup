          ...
          <div className="mb-4 flex w-full flex-col rounded-2xl bg-popover p-5 md:flex-row ">
            <Accordion type="multiple" className="w-full rounded-2xl bg-popover p-5">
              {flights.map(({ flightData, id }) => {
                return (
                  <AccordionItem value="flight" className="flex flex-col" key={flightData.state.selectedTicket.id}>
                    {flightData.state.selectedTicket.legs.map((leg) => (
                      <AccordionTrigger key={leg.id}>
                        <TicketCardLeg leg={leg} className="w-full pr-10 lg:[&>*:last-child]:w-full" />
                      </AccordionTrigger>
                    ))}
                    <AccordionContent className="px-4">
                      ...
                      <div className="flex items-center justify-between gap-4 px-5 pt-4 ">
                        <div className="flex gap-2">
                          <p className="text-lg font-medium text-white">Всього:</p>
